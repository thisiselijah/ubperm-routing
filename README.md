...
5. **Algebraically-Informed Stochastic Search:** Combines the exact mathematical heuristics of Entropy-Cycle with a Softmax probabilistic sampling (Gibbs/Simulated Annealing) over state transitions. 
   * **Memory & Time:** Unlike A*, which maintains a massive priority queue ($O(b^D)$ memory), Stochastic Search has an **$O(1)$ memory footprint** and scales elegantly to massively dense networks (e.g., N=64 or N=256), gracefully side-stepping the curse of dimensionality at the cost of slight long-tail deviations in path optimality.
...
# Run
./src/bin/routing -algo <bfs|merge|astar|entropy_cycle|stochastic> <-default|-test> <comma_separated_numbers>
...
- `-algo <string>`: Algorithm to use (`bfs`, `merge`, `astar`, `entropy_cycle`, `beam_search`, `stochastic`).
...
### 3. Random Testing Script (`run_random.py`)
Generates a random permutation of a specified hypercube dimension and runs a single routing test on it. Extremely useful for testing algorithms on higher dimensions (e.g., N=32 or N=64) where exhaustive evaluation is impossible.
```bash
# Generate a random 5-cube (N=32) permutation and route it using Stochastic Search
python scripts/run_random.py -cube 5 -algo stochastic
```
**Arguments:**
- `-cube <int>`: Hypercube dimension (e.g., `5` for 32 nodes).
- `-exe <path>`: Path to the C++ executable (default: `src/bin/routing`).
- `-algo <string>`: Algorithm to use (default: `stochastic`).

### 4. Plotting Script (`plot.py`)
...
### 5. WebAssembly Module Compilation
